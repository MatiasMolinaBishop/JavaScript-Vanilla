...
for (const flight of flights.split("+")) {
  //console.log(flight);
  const [bound, from, to, time] = flight.split(";");
  const output = `${
    bound.startsWith("_Delayed") ? "👽" : ""
  } ${bound.replaceAll("_", " ")}from ${
    from[0].toLocaleUpperCase() +
    from[1].toLocaleUpperCase() +
    from[2].toLocaleUpperCase()
  } to ${to.slice(0, 3).toLocaleUpperCase()} (${time.replace(
    ":",
    "h"
  )})`.padStart(36);
  console.log(output);
